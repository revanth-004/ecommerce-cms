...
import ArticleCategoriesForm from "../../components/form/ArticleCategoriesForm";
...
const buildTree = (items) => {
  ...
  items.forEach((item) => {
    if (item.articleCategoryParentId && map[item.articleCategoryParentId]) {
      map[item.articleCategoryParentId].children.push(map[item._id]);
    } else {
...
const ArticleCategories = () => {
  ...
  const handleEdit = (record) => {
    setFormData({
      ...
      articleCategoryTitle: record.articleCategoryTitle,
      articleCategoryLogo: record.articleCategoryLogo,
      articleCategoryParentId: record.articleCategoryParentId || "",
    });
  ...
  const fetchCategories = () => {
    ...
    axios
      .get(
        `http://localhost:3000/api/articleCategory?companyId=${selectedCompany._id}`,
      )
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:3000/api/articleCategory/${id}`,
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.articleCategoryTitle)
      errors.articleCategoryTitle = "Category title is required";
    if (!logoFile && !formData.articleCategoryLogo)
      errors.articleCategoryLogo = "Logo is required";
    ...
    try {
      ...
      if (logoFile) {
        ...
        payload.articleCategoryLogo = res.data.filePath;
      }

      if (editingRecord) {
        const response = await axios.put(
          `http://localhost:3000/api/articleCategory/${editingRecord._id}`,
          payload,
        );
        showToast(response.data.message, "success");
      } else {
        const response = await axios.post(
          "http://localhost:3000/api/articleCategory",
          payload,
        );
        showToast(response.data.message, "success");
      }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="relative w-full max-w-2xl rounded-xl bg-white p-6 shadow-xl">
            ...
            <h2 className="text-lg font-semibold mb-4">
              {editingRecord ? "Edit Article Category" : "Add Article Category"}
            </h2>
